 ...
 */
public class UsuarioGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all usuarios").get("/api/usuarios").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new usuario")
                        .post("/api/usuarios")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombreUsuario\": \"SAMPLE_TEXT\"" + ", \"rol\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_usuario_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created usuario").get("${new_usuario_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created usuario").delete("${new_usuario_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Usuario entity").exec(scn);
