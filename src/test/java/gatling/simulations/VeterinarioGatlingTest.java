...
/**
 * Performance test for the Veterinario entity.
 */
public class VeterinarioGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all veterinarios").get("/api/veterinarios").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new veterinario")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"especilidad\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_veterinario_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created veterinario").get("${new_veterinario_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created veterinario").delete("${new_veterinario_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Veterinario entity").exec(scn);
