...
/**
 * Performance test for the Factores entity.
 */
public class FactoresGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all factores").get("/api/factores").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new factores")
                        .post("/api/factores")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_factores_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created factores").get("${new_factores_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created factores").delete("${new_factores_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Factores entity").exec(scn);
