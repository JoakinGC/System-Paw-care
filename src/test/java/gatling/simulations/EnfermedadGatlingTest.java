...
/**
 * Performance test for the Enfermedad entity.
 */
public class EnfermedadGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all enfermedads").get("/api/enfermedads").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new enfermedad")
                        .post("/api/enfermedads")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + ", \"descripcion\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_enfermedad_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created enfermedad").get("${new_enfermedad_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created enfermedad").delete("${new_enfermedad_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Enfermedad entity").exec(scn);
