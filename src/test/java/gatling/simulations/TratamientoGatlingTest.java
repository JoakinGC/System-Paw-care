 ...
 */
public class TratamientoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all tratamientos").get("/api/tratamientos").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tratamiento")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created tratamiento").get("${new_tratamiento_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created tratamiento").delete("${new_tratamiento_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Tratamiento entity").exec(scn);
