 ...
 */
public class EstudiosGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all estudios").get("/api/estudios").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new estudios")
                        .post("/api/estudios")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"nombre\": \"SAMPLE_TEXT\"" +
                                ", \"fechaCursado\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"nombreInsituto\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_estudios_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created estudios").get("${new_estudios_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created estudios").delete("${new_estudios_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Estudios entity").exec(scn);
