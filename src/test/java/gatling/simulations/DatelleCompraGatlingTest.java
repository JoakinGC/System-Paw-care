...
/**
 * Performance test for the DatelleCompra entity.
 */
public class DatelleCompraGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all datelleCompras").get("/api/datelle-compras").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new datelleCompra")
                        .post("/api/datelle-compras")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"cantidad\": 0" + ", \"precioUnitario\": 0" + ", \"totalProducto\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_datelleCompra_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created datelleCompra").get("${new_datelleCompra_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created datelleCompra").delete("${new_datelleCompra_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DatelleCompra entity").exec(scn);
