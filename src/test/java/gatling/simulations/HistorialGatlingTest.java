...
/**
 * Performance test for the Historial entity.
 */
public class HistorialGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all historials").get("/api/historials").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new historial")
                        .post("/api/historials")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody("{" + "\"fechaConsulta\": \"2020-01-01T00:00:00.000Z\"" + ", \"diagnostico\": \"SAMPLE_TEXT\"" + "}")
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_historial_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created historial").get("${new_historial_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created historial").delete("${new_historial_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Historial entity").exec(scn);
