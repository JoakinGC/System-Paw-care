...
/**
 * Performance test for the Raza entity.
 */
public class RazaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all razas").get("/api/razas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new raza")
                        .post("/api/razas")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + ", \"nombreCientifico\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_raza_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created raza").get("${new_raza_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created raza").delete("${new_raza_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Raza entity").exec(scn);
