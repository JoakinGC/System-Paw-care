...
/**
 * Performance test for the Especie entity.
 */
public class EspecieGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all especies").get("/api/especies").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new especie")
                        .post("/api/especies")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + ", \"nombreCientifico\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_especie_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created especie").get("${new_especie_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created especie").delete("${new_especie_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Especie entity").exec(scn);
