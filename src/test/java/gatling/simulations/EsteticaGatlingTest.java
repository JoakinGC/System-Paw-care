...
/**
 * Performance test for the Estetica entity.
 */
public class EsteticaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all esteticas").get("/api/esteticas").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new estetica")
                        .post("/api/esteticas")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_estetica_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created estetica").get("${new_estetica_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created estetica").delete("${new_estetica_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Estetica entity").exec(scn);
