...
/**
 * Performance test for the Terapia entity.
 */
public class TerapiaGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all terapias").get("/api/terapias").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new terapia")
                        .post("/api/terapias")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nombre\": \"SAMPLE_TEXT\"" + ", \"descripcion\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_terapia_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created terapia").get("${new_terapia_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created terapia").delete("${new_terapia_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Terapia entity").exec(scn);
