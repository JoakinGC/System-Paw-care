...
/**
 * Performance test for the CuidadoraHotel entity.
 */
public class CuidadoraHotelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all cuidadoraHotels").get("/api/cuidadora-hotels").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cuidadoraHotel")
                        .post("/api/cuidadora-hotels")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"servicioOfrecido\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cuidadoraHotel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created cuidadoraHotel").get("${new_cuidadoraHotel_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created cuidadoraHotel").delete("${new_cuidadoraHotel_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CuidadoraHotel entity").exec(scn);
