 ...
 */
public class CompraGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all compras").get("/api/compras").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new compra")
                        .post("/api/compras")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"fechaCompra\": \"2020-01-01T00:00:00.000Z\"" + ", \"total\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_compra_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created compra").get("${new_compra_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created compra").delete("${new_compra_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Compra entity").exec(scn);
